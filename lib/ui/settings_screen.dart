/*
 *  settings_screen.dart  (v3 – lokalisiert)
 *  --------------------------------------------------------------
...
class _SettingsBodyState extends State<_SettingsBody> {
  ...
  void initState() {
    super.initState();
    schema = loadYaml(
        File('assets/config/settings_schema.yaml').readAsStringSync()) as Map;
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        children: [
          ...
          _dropdown(
              l.labelSpeechMode, adapter.speechMode, adapter.setSpeechMode, ['offline', 'hybrid', 'online']),
          _dropdown(
              l.labelImageMode, adapter.imageMode, adapter.setImageMode, ['offline', 'hybrid', 'online']),
          ...
          Center(
            child: TextButton(
              onPressed: () => launchUrl(Uri.parse('https://github.com/nightscout/androidaps')),
  ...
  Widget _textField(
      String label, String val, Future<void> Function(String) setter) =>
      ListTile(
        title: Text(label),
        subtitle: TextFormField(
          initialValue: val,
          onFieldSubmitted: setter,
        ),
      );

  Widget _numberField(
      String label, int val, Future<void> Function(int) setter) =>
      ListTile(
        title: Text(label),
        subtitle: TextFormField(
          initialValue: '$val',
          keyboardType: TextInputType.number,
          onFieldSubmitted: (v) => setter(int.tryParse(v) ?? val),
        ),
      );

  Widget _dropdown(String label, String current, Function(String?) onChanged,
      List<String> opts) {
    return ListTile(
      title: Text(label),
  ...
  Widget _boolSwitch(
      String label, bool val, Future<void> Function(bool) setter) =>
      SwitchListTile(
        title: Text(label),
        value: val,
        onChanged: setter,
  ...
  Future<void> _pickAvatarItem(BuildContext ctx) async {
    ...
    if (bytes.lengthInBytes > 200 * 1024) {
      ScaffoldMessenger.of(ctx)
          .showSnackBar(SnackBar(content: Text(l.msgFileTooBig)));
      return;
    }
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(ctx)
        .showSnackBar(SnackBar(content: Text(l.msgItemAdded)));
  }
}
...
class _SettingsAdapter extends ChangeNotifier {
  ...
  void setNsUrl(String v) => _set(() => s.setNightscoutUrl(v));
  void setNsSecret(String v) => _set(() => s.setNightscoutSecret(v));
  void setGpt(String v) => _set(() => s.setGptApiKey(v));
  void setWhisper(String v) => _set(() => s.setWhisperApiKey(v));
  void setVision(String v) => _set(() => s.setVisionApiKey(v));

  void setSpeechMode(String? v) => _set(() => s.setSpeechMode(v ?? s.speechMode));
  void setImageMode(String? v) => _set(() => s.setImageMode(v ?? s.imageMode));

  void setPush(bool v) => _set(() => s.setEnablePush(v));
  void setSms(bool v) => _set(() => s.setEnableSms(v));
  void setMute(bool v) => _set(() => s.setMuteAlarms(v));
  void setPhone(String v) => _set(() => s.setParentPhone(v));

  void setPpMeal(int v) => _set(() => s.setPointsPerMeal(v));
  void setPpSnack(int v) => _set(() => s.setPointsPerSnack(v));
  void setBonusSnacks(int v) => _set(() => s.setBonusEverySnacks(v));

  void setCarbWarn(int v) => _set(() => s.setCarbWarnThreshold(v));

  void setTheme(String? v) {
    if (v != null) _set(() => s.setChildTheme(v));
  }

  void _set(Future<void> Function() f) async {
    await f();
    notifyListeners();
  }
}
