/*
 *  settings_service.dart  (v7 – FINAL + SETUP SUPPORT)
 *  --------------------------------------------------------------
 ...
 *  • NEU: Support für initialSetupDone + resetSetup
 *
...
class SettingsService {
  ...
  Future<void> resetSetup() async => await _prefs.setBool('kidsapp_setup_done', false);
  ...
  Future<void> addPoints(int delta) async =>
      _set('kidsapp_points', childPoints + delta, silent: true)
          .then((_) => _set('kidsapp_level', 1 + (childPoints + delta) ~/ 100, silent: true));
  ...
  List<String> get unlockedGimmicks => _prefs.getStringList('kidsapp_gimmicks') ?? <String>[];
  Future<void> unlockGimmick(String k) async => _set('kidsapp_gimmicks', {...unlockedGimmicks, k}.toList());
  ...
  static const MethodChannel _prefBridge = MethodChannel('kidsapp/settings_bridge');

  Future<void> _set(String key, Object value, {String? mirrorKey, bool silent = false}) async {
    if (value is int) {
    ...
    if (flavor == AppFlavor.plugin && mirrorKey != null) {
      try {
        await _prefBridge.invokeMethod('setPref', {'key': mirrorKey, 'value': value});
      } catch (_) {/* ignore */}
    }
