/*
 *  meal_analyzer.dart  (v7 – i18n NAMESPACED)
 *  --------------------------------------------------------------
...
class MealAnalyzer {
  ...
  Future<void> analyze(String input) async {
    ...
    _processWarnings(totalCarbs, fuzzyHitOccurred);
    await _sync.persistMeal(totalCarbs, results);
    ...
    if (_settings.insulinRatio > 0) {
      ...
      _bus.fire(BolusCalculatedEvent(
        carbs: totalCarbs,
        units: units,
        reason: LocalizationHelper.get('carb_analysis.reason.default', placeholders: {
          'carbs': _round(totalCarbs).toString(),
          'ratio': _settings.insulinRatio.toString()
        }),
        isSafe: units < 10,
      ));
    }
  ...
  void _processWarnings(double totalCarbs, bool fuzzy) {
    final warnThreshold = _settings.carbWarnThreshold;
    final List<String> warns = [];

    if (totalCarbs >= warnThreshold) {
      warns.add(LocalizationHelper.get('carb_analysis.warn.high',
          placeholders: {'carbs': _round(totalCarbs).toString()}));
    }
    if (fuzzy) warns.add(LocalizationHelper.get('carb_analysis.warn.fuzzy'));
    if (totalCarbs > 250) warns.add(LocalizationHelper.get('carb_analysis.warn.excessive'));
